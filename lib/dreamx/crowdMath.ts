...
export function calculateFollowerDelta(params: FollowerGrowthParams): FollowerGrowthResult {
  ...
  let rawDelta = (baseGrowth + viralGrowth) * saturationFactor * controversyMultiplier;
...
export function estimatePostEngagement(params: PostEngagementParams): PostEngagementResult {
  ...
  // A percentage of followers see it, boosted by momentum and controversy.
  const reachPercentage = clamp(0.1 + (controversy * 0.1) + (networkMomentum * 0.05), 0.05, 0.8);
  const baseImpressions = authorFollowers * reachPercentage;
